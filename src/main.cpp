...
uint256 hashGenesisBlock("0x0000036fac9736f500d75c391b6fb998dcae07ba7f012e5827fda73b41786b86");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // RitzcoinPlus: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "RitzcoinPlus Signed Message:\n";
...
{
    // RitzcoinPlus: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula - DarkGravity v3 */
...
{
    // RitzcoinPlus: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {	
		const char* pszTimestamp = "RTCP Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1473903528;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 188288;

        if (fTestNet)
        {
            block.nTime    = 1394733194;
            block.nNonce   = 1284927360;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xf353c98071328e1431f2c721fa5e504c7b290c99edce05912787ba3d003301ec"));
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb7, 0xda }; // RitzcoinPlus: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RitzcoinPlusMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RitzcoinPlusMiner : ProcessBlock, block not accepted");
    }
